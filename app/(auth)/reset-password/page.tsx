...
function ResetForm() {
  ...
  const validate = useCallback(async () => {
    if (!token) { setTokenError("No reset token found in link."); setValidating(false); return; }
    const res = await validateResetToken(token);
    setValidating(false);
    if (res.success) { setTokenValid(true); }
    else { setTokenError(res.message); }
  }, [token]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await saveNewPassword(token, password);
    setLoading(false);

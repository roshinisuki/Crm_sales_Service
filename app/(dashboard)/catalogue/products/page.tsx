  ...
  return (
    <PageContainer>
      <div className="mb-6">
        ...
        <p className="text-gray-400 mt-1">Manage your product catalogue</p>
      </div>

      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col gap-4 md:flex-row md:items-center flex-1">
          <div className="relative flex-1 max-w-md">
            <Ico d={icons.search} size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input
              type="text"
              placeholder="Search products..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-white/10 bg-white/5 text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
            />
          </div>
          <select
            value={categoryId}
            onChange={(e) => setCategoryId(e.target.value)}
            className="px-4 py-2.5 rounded-lg border border-white/10 bg-white/5 text-white focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-400">No products yet. Add your first product to the catalogue.</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Product Code</th>
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-400">Base Price</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-400">Active</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-400">Documents</th>
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">{product.basePrice ? `₹${product.basePrice.toFixed(2)}` : "-"}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {!product.datasheetUrl && !product.brochureUrl && <span className="text-gray-500">-</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        title="View Details"
                      >
                        <Ico d={icons.external} size={16} className="text-gray-400" />
                      </button>
                      <button
                        onClick={() => handleToggleActive(product.id, product.isActive)}
                        className={`p-1.5 rounded hover:bg-white/10 transition-colors ${product.isActive ? "text-amber-600" : "text-emerald-600"}`}

  ...
  const load = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        setBrochureUrl(data.data.brochureUrl || "");
      } else {
  ...
  return (
    <PageContainer>
      <div className="mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Brochures</h1>
        ...
        {editing ? (
          <div className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-slate-500 mt-1.5">Link to a PDF or external document hosting the product brochure.</p>
            </div>
            <div className="flex gap-3">
              ...
              <button
                onClick={() => { setEditing(false); setBrochureUrl(product.brochureUrl || ""); }}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-800">Current Brochure</h2>
              <button
            ...
            {brochureUrl ? (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
                <Ico d={icons.file} size={24} className="text-purple-600" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 truncate">{brochureUrl.split("/").pop() || "Brochure"}</p>
                  <p className="text-xs text-slate-500 truncate">{brochureUrl}</p>
                </div>
                <a
                  href={brochureUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-600 text-white text-sm font-medium hover:opacity-90 transition-opacity"
                >
              ...
              <div className="text-center py-8">
                ...
                <p className="text-slate-500 text-sm">No brochure linked to this product yet.</p>
                <button
                  onClick={() => setEditing(true)}

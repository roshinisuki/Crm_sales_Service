...
export default function WhatsAppTemplatesPage() {
  ...
  const [form, setForm] = useState({ name: "", message: "", module: "Quotation", isActive: true });
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/whatsapp-templates?${new URLSearchParams(params)}`);
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ name: "", message: "", module: "Quotation", isActive: true });
    setEditorOpen(true);
  };

  const openEdit = (t: any) => {
    setEditing(t);
    setForm({ name: t.name, message: t.message, module: t.module, isActive: t.isActive });
    setEditorOpen(true);
  };
    ...
    setConfirmState({
      ...
      action: async () => {
        try {
          ...
          const data = await res.json();
          if (data.success) { toast.success("Template deleted"); load(); }
    ...
    <PageContainer>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage WhatsApp message templates for quick customer communication</p>
        </div>
        <button onClick={openNew} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
          <Ico d={icons.plus} size={16} /> New Template
        </button>
      </div>

      <div className="flex flex-wrap gap-2 mb-4">
        <button onClick={() => setModuleFilter("")} className={`px-3 py-1.5 rounded-full text-xs font-medium ${!moduleFilter ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>All</button>
        {modules.map(m => (
          <button key={m} onClick={() => setModuleFilter(m)} className={`px-3 py-1.5 rounded-full text-xs font-medium ${moduleFilter === m ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>{m}</button>
        ))}
      </div>
            ...
            <div key={t.id} className="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition-shadow">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  ...
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${moduleColors[t.module] || moduleColors.General}`}>{t.module}</span>
                </div>
                {!t.isActive && <span className="text-xs text-gray-400">Inactive</span>}
              </div>
              <p className="text-xs text-gray-600 line-clamp-4 mb-3 whitespace-pre-wrap">{t.message.substring(0, 200)}{t.message.length > 200 ? "..." : ""}</p>
      ...
      {editorOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editing ? "Edit Template" : "New WhatsApp Template"}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Template Name *</label>
                <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Quotation Follow-up" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.message} onChange={e => setForm({...form, message: e.target.value})} rows={6} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="WhatsApp message body. Use {{variable}} for dynamic content." />
                <p className="text-xs text-gray-400 mt-1">Keep messages concise. Available: {`{{customerName}}, {{dealName}}, {{quoteCode}}, {{poCode}}, {{amount}}, {{date}}, {{userName}}`}</p>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
              ...
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50">{saving ? "Saving..." : "Save Template"}</button>
            </div>
          </div>
        </div>
      )}

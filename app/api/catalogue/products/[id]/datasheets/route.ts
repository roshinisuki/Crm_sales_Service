...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: datasheets });
  } catch (error) {
    console.error("GET /api/catalogue/products/[id]/datasheets error:", error);
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: datasheet }, { status: 201 });
  } catch (error) {
    console.error("POST /api/catalogue/products/[id]/datasheets error:", error);
    return NextResponse.json({ success: false, message: "Internal server error" }, { status: 500 });
  }
}

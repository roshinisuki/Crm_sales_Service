...
export async function GET(request: NextRequest) {
  ...
  const guard = enforceModuleGuard(user, MODULE_KEYS.CUSTOMER_VISITS, "GET /api/visits");
...
export async function POST(request: NextRequest) {
  ...
  const guard = enforceModuleGuard(user, MODULE_KEYS.CUSTOMER_VISITS, "POST /api/visits");
  if (guard) return guard;
